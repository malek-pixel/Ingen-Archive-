/**
 * Ingests botanical imagery.
 *
 * Reads any image dropped into incoming/flora/, converts it to WebP at the
 * same 1200px cap the record plates use, writes it to public/media/flora/,
 * and records the path in src/data/flora-images.json keyed by record id.
 *
 * Partial coverage is expected and supported: a botanical record with no file keeps its
...
const inbox = join(root, "incoming/flora");
const outDir = join(root, "public/media/flora");
const mapPath = join(root, "src/data/flora-images.json");

const flora = JSON.parse(readFileSync(join(root, "src/data/flora.json"), "utf8"));
const validIds = new Set(flora.map((f) => f.id));
...
for (const file of files) {
  ...
  map[id] = { src: `/media/flora/${id}.webp`, w: width, h: height };
  unlinkSync(src);
}
...
if (unknown.length) {
  console.warn(`\nignored — filename is not a flora record id:\n  ${unknown.join("\n  ")}`);
  console.warn(`\nvalid ids:\n  ${[...validIds].join("\n  ")}`);
}

const withImage = Object.keys(map);
const withoutImage = [...validIds].filter((id) => !map[id]);
console.log(`\n${withImage.length}/${validIds.size} botanical records have imagery`);
